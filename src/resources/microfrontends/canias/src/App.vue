<template>
  <div class="flex flex-col bg-[#c0d0e0] h-screen">
    ...
    <!-- <div
      class="flex items-center gap-4 bg-[#e0e8f0] px-4 py-2 border-[#8090a0] border-b-2"
    >
    ...
    <!-- Tabs Bar -->
    <div class="flex bg-[#b0c0d0] border-gray-400 border-b h-10">
      <div
      ...
      <select
        v-model="selectedPage"
        @change="openPage"
        class="top-1 right-2 absolute bg-white shadow-sm px-3 py-1 border border-gray-400 rounded focus:outline-none focus:ring-2 focus:ring-blue-400 text-sm"
      >
        <option value="">Select a page...</option>
        <option v-for="page in availablePages" :key="page.id" :value="page.id">
          {{ page.title }}
    ...
    <div class="flex-1 overflow-hidden">
      <component v-if="activeTab" :is="activeTab.component" class="h-full" />
      <div v-else class="flex justify-center items-center h-full text-gray-500">
        <p class="text-lg">
          Select a page from the dropdown above to get started
        </p>
      </div>
    </div>
  </div>
